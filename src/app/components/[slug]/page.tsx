...
export default function SlugDetailPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const { slug } = use(params);
  // Detect type from URL path (set by parent route)
  ...
  // Detect type from current URL path
  const path = typeof window !== "undefined" ? window.location.pathname : "";
  const type = path.startsWith("/templates/") ? "template" :
              path.startsWith("/components/") ? "component" :
              path.startsWith("/assets/") ? "asset" :
              path.startsWith("/skills/") ? "skill" : "template";
  ...
  const backUrl = item.type === "template" ? "/?tab=templates" : item.type === "component" ? "/?tab=components" : item.type === "asset" ? "/?tab=assets" : "/?tab=skills";
            ...
            {contentStatus === "notfound" && (
              <div className="artifact-empty">
                <div className="artifact-empty-content">
                  ...
                  <p>{tab === "design" || tab === "prompt" ? "This artifact is being generated in the background. Check the Progress tab for status." : "The code for this item is not available."}</p>
                </div>
              </div>
            )}
            {contentStatus === "error" && (
              <div className="artifact-empty">
                <div className="artifact-empty-content">
                  ...
                  <p>There was an error fetching this content.</p>
